...
def normalize_report_rows(records):
    ...
    for r in records:
        ...
        # Explicit function to pick correct score (0 is valid!)
        def pick_score(r):
...
class WeeklyReportView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            result = []
            for p in ranked:
                manager_obj = getattr(p.employee, "manager", None)
                manager_full_name = (
            ...
            return Response(
                {
                    ...
                    "records": normalize_report_rows(WeeklyReportSerializer(result, many=True).data),
                },
                status=status.HTTP_200_OK,
            )
